...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=['MODEL.WEIGHTS', '../model_0019999.pth', 'INPUT.MIN_SIZE_TEST', 3240, 'INPUT.MAX_SIZE_TEST', 9999, 'TEST.DETECTIONS_PER_IMAGE', 1000],
...
model_ft.load_state_dict(torch.load("../checkpoint1.pth"))
...
def upload(thres):
    if request.method == 'POST':
        ...
        os.mkdir(os.path.join(folder_name, 'cells'))
        ...
        for dscore, box in zip(det_scores, boxes):
            ...
            if x < 0.01:
                x = 0
            scores.append(str(x)[:4])
            if x > thres:
                mask[y0:y1,x0:x1] = im[y0:y1,x0:x1]
            cv2.imwrite(os.path.join(folder_name, 'cells', '%.2f_%d_%d.png'%(x, y0, x0)), im[y0:y1,x0:x1,::-1])
        ...
        vis0.draw_instance_predictions(boxes, scores)
        vis0.get_output().save('static/images/'+name_str+'_cls.png')
        vis0.get_output().save(os.path.join(folder_name, 'prediction.png'))
